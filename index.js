...
Fases: 0(Infra), 1(Catálogos), 2(Sucursales), 3(Seguridad), 3b(Fix Passwords),
       4(Productos), 4b(Fix Precios), 4c(Fix USD), 5(Clientes), 6(Red MLM),
       7(Ventas), 7b(Tipos Orden), 8(Facturación), 9(Comisiones),
       10(Inventario), 10b(Proveedores), 11(RRHH), 12(Comunicación),
       13(Auditoría), 99(Post-Migración)
...
const DEFAULT_ORDER = [0, 1, 2, 3, '3b', 4, '4b', '4c', 5, 6, 7, '7b', 8, 9, 10, '10b', 11, 12, 13, 14, 99];
...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--phase' || arg === '-p') {
      const val = args[++i];
      if (val !== undefined) {
        const phaseKey = (val === '10b' || val === '3b' || val === '4b' || val === '4c' || val === '7b') ? val : Number(val);
        if (PHASES[phaseKey]) {
